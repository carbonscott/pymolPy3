...
# Disulfide bond
pm(f"hide cartoon, ICL1 or ICL2 or ICL3 or ECL1 or ECL3")
pm(f"select ds_bond, resi 110 or resi 187")
pm(f"show sticks, %ds_bond")

# Asp190
pm(f"select asp190, resi 190")
pm(f"show sticks, %asp190")

# Hydrogen bond network
pm(f"select h_bond_net, resi 181 or resi 192 or resi 268 or resi 186 or resi 113 or resi 187 or resi 94")
pm(f"show sticks, %h_bond_net")
pm(f"select glu181, resi 181")
pm(f"select tyr192, resi 192")
pm(f"select tyr268, resi 268")
pm(f"select ser186, resi 186")
pm(f"select glu113, resi 113")
pm(f"select cys187, resi 187")
pm(f"select  thr94, resi  94")

# Highlight hbond regions for inactive
pm(f"hide cartoon, average")
pm(f"set cartoon_transparency, 0.8, active or inactive")
pm(f"hide sticks, ret_active or ret_inactive")
pm(f"hide sticks, asp190")
pm(f"set cartoon_transparency, 0.2, %ECL2")

pm(f"set_view (\\")
pm(f"    -0.625136316,   -0.391858429,   -0.674971938,\\")
pm(f"     0.656323612,    0.204034939,   -0.726348996,\\")
pm(f"     0.422345638,   -0.897080839,    0.129652619,\\")
pm(f"     0.001755327,    0.000253782,  -81.057243347,\\")
pm(f"    52.984703064,   10.320146561,  -14.370937347,\\")
pm(f"   -13.183941841,  176.004104614,  -20.000000000 )")
